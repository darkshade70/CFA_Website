...
export async function POST(req: Request) {
  try {
    const { name, phone, email, program, which, experience, comments } = await req.json();
    ...
    const { error } = await resend.emails.send({
      ...
      to: [TO],
      replyTo: email,
      subject: `Registration interest — ${name}`,
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:580px;margin:0 auto;border-radius:8px;overflow:hidden;border:1px solid #eee;">
          <div style="background:#800017;padding:28px 32px;">
            ...
            <p style="color:#fff;font-size:22px;font-weight:700;margin:0;">New registration interest</p>
          </div>
          <div style="padding:32px;background:#fff;">
            <table style="width:100%;border-collapse:collapse;font-size:15px;line-height:1.6;">
              ...
              <tr>
                <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;font-weight:600;color:#555;">Program</td>
                <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;color:#111;">${program}</td>
              </tr>
              <tr>
                <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;font-weight:600;color:#555;">Discipline</td>
                <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;color:#111;">${which}</td>
              </tr>
              <tr>
                <td style="padding:10px 0;${comments ? "border-bottom:1px solid #f0f0f0;" : ""}font-weight:600;color:#555;">Experience</td>
                <td style="padding:10px 0;${comments ? "border-bottom:1px solid #f0f0f0;" : ""}color:#111;">${experience}</td>
              </tr>
