...
function RegisterForm() {
  ...
  // Read pre-fill params from program page links
  const paramWhich = searchParams.get("which") ?? "";
  const paramProgram = searchParams.get("program") ?? "";
  const paramClass = searchParams.get("class") ?? "";

  const initialWhich = WHICH.includes(paramWhich) ? paramWhich : "All / Not sure";
  const initialProgram = PROGRAMS.includes(paramProgram) ? paramProgram : "Youth programs";

  // If a class was passed and no program was inferred, keep default
  const [form, setForm] = useState({
    ...
    comments: paramClass ? `Interested in: ${paramClass}` : "",
  });
...
export default function RegisterPage() {
  return (
    <>
      ...
      {/* ── Form + sidebar — wrapped in Suspense for useSearchParams ── */}
